...
import { accountIsBlocked } from "../api-client-utils";
import { number, boolean, string } from "@pact-foundation/pact/src/v3/matchers";
  ...
  it('will accept a GET request and return data for an existing user', async () => {
    provider.addInteraction({
      ...
      willRespondWith: {
        ...
        body: {
          intervention: {
            updatedAt: number(123_455),
            appliedAt: number(12_345_685_809),
            sentAt: number(123_456_789),
            description: string('AIS_ACCOUNT_BLOCKED'),
          },
          state: {
