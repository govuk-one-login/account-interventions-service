...
describe('event-validation', () => {
  ...
  it('should return an error as intervention is invalid', () => {
    ...
    expect(() => validateEventAgainstSchema(TxMAEvent as TxMAEvent)).toThrow(new ValidationError('Invalid intervention event.'));
  ...
  it('should throw an error if event is stale', async () => {
    ...
    await expect(async () =>
      await validateEventIsNotStale(EventsEnum.FRAUD_SUSPEND_ACCOUNT, staleEvent, dynamoDBResult)
    ...
    expect(sendAuditEvent).toHaveBeenCalledWith("AIS_INTERVENTION_IGNORED_STALE", "urn:fdc:gov.uk:2022:USER_ONE", {"appliedAt": undefined, "intervention": "FRAUD_SUSPEND_ACCOUNT", "reason": "Received intervention predates latest applied intervention"})
  });
  ...
  it('should not throw if event is not stale', async () => {
    ...
    await expect(validateEventIsNotStale(EventsEnum.FRAUD_SUSPEND_ACCOUNT, nonStaleEvent, dynamoDBResult)).resolves.toEqual(undefined);
  ...
  it('should throw an error if event timestamp is in the future', async () => {
    ...
    await expect(async () => { await validateEventIsNotInFuture(EventsEnum.FRAUD_SUSPEND_ACCOUNT, eventInTheFuture)}).rejects.toThrow(new Error("Event is in the future. It will be retried"));
  ...
  it('should not throw an error if the event is not in the future', async () => {
    ...
    await expect(validateEventIsNotInFuture(EventsEnum.FRAUD_SUSPEND_ACCOUNT, eventNotInTheFuture)).resolves.toEqual(undefined);
    expect(logAndPublishMetric).not.toHaveBeenCalled();
    expect(sendAuditEvent).not.toHaveBeenCalled();
  });
