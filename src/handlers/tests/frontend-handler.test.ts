...
describe('frontend-handler', () => {
  ...
  describe('redirect check', () => {
    it('returns a 302 redirect when redirect is true', async () => {
      const { handler } = await import('../frontend-handler');
      const event = makeEvent('/some/path');
    ...
    it('passes through to the proxy when redirect is false', async () => {
      const { handler } = await import('../frontend-handler');
      const event = makeEvent('/');
      event.requestContext = {
        ...event.requestContext,
    ...
    it('should return 302 with empty headers when authoriser values are not strings', async () => {
      const { handler } = await import('../frontend-handler');
      const event = makeEvent('/');
      event.requestContext = {
        ...event.requestContext,
