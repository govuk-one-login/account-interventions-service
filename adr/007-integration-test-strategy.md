...
At all system boundaries, e.g. inputs and outputs of Lambdas, reads and writes to DynamoDB, we will validate data using `Zod` or `ajv` and enforce strict types.
Where the system boundary is external, the types must be public within the programme, e.g. types from the event catalogue.
We will produce OpenAPI specs for any HTTP APIs.

> NB Testing infrastructure is out of scope for this document; it will be addressed in a future RFC or ADR.
...
For further discussion of these options, see [the Notes section](#notes).
A mixture of options 1 and 3 as described in [the What section](#what) offers the best balance of pre-merge simplicity and speed, and validation of the application as a whole before it gets to production.
...
1. **Do not delay route to live:** Because the application interfaces are robustly checked at build and test-time, post-merge checks are only needed to validate assumptions about the AWS resources. This means we only need minimal happy-path tests, reducing the test burden in intermediate environments and keeping the route to live fast.
...
These stacks also take time to deploy and cost money to run.
As a lower bound for the time to deploy a stack for the AIS application, the deployment time for the main stack on dev (i.e. deploying to a stack that already exists) is approximately 4 minutes.
Considered as a potential delay in the route to production this is ~10% of the 43-minute monthly error budget based on a 99.9% uptime SLA.
...
Dependencies on external solutions can also lead to availability issues; for example, LocalStack is deprecating its free community version.
...
Where the data is coming from outside the system, we must provide shared types to both systems using an external source (e.g. the event catalogue) or provide a strict interface (e.g. an OpenAPI spec).

The benefits of this approach are essentially the opposite of the downsides of Option 1 - we can rely on a component-based test suite which is small and fast, we don't have to manage AWS resources in CI, and we don't unnecessarily test the code.
...
### Option 4: A balanced blend

To maximise the upsides and mitigate the downsides of these options we've gone with a blend of options 1 and 3.
The full approach is described in the [What section](#what) but broadly we use option 3 plus run option 1-style integration tests post-merge to validate our assumptions about AWS resources.
